...
export async function leaveRoom(){
  try{
    ...
    // выключаем треки мягко
    try{ if(lp?.setMicrophoneEnabled) await lp.setMicrophoneEnabled(false); }catch{}
    try{ if(lp?.setCameraEnabled)     await lp.setCameraEnabled(false); }catch{}
    try{ window.localVideoTrack?.stop?.(); }catch{}
    try{ window.localAudioTrack?.stop?.(); }catch{}

    // останавливаем шэринг, если был
    try{
      if(Array.isArray(window.screenTracks)){
        for(const t of window.screenTracks){
          try{ await window.room?.localParticipant?.unpublishTrack?.(t); }catch{}
          try{ t.stop?.(); }catch{}
        }
      }
    }catch{}
    ...
    // отключаемся
    if(window.room){ try{ await window.room.disconnect(); }catch{} }
  }finally{
    // UI-сброс
    await exitFullscreenAndUnlock();
    byId('onlineList')?.replaceChildren();
    byId('tilesMain')?.replaceChildren();
    byId('tilesRail')?.replaceChildren();

    // возвращаем экран логина
    show('screen-auth');
    hide('screen-app');

    // звукошум (опционально)
    try{ sfxLeave?.(); }catch{}
  }
}
